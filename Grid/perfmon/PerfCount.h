...
public:
  ...
  static int NumTypes(void) { return PERFORMANCE_COUNTER_NUM_TYPES; }
  ...
  void Open(void) {
    ...
    fd = perf_event_open(&pe, 0, -1, -1,
                         0); // pid 0, cpu -1 current process any cpu. group -1
    if (fd == -1) {
    ...
    cyclefd = perf_event_open(
        &pe, 0, -1, -1, 0); // pid 0, cpu -1 current process any cpu. group -1
    if (cyclefd == -1) {
      fprintf(stderr, "Error opening leader %llx for event %s\n",
              (long long)pe.config, name);
      perror("Error is");
  ...
  void Report(void) {
    ...
    std::printf("tsc = %llu %s = %llu  %s = %20llu\n (%s/%s) rate = %lf\n",
                elapsed, sn, cycles, sc, count, sc, sn,
