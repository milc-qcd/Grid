...
void benchSt(Coordinate & L, int Ls);
...
int main (int argc, char ** argv)
{
...
#else
  std::vector<Coordinate> latts;
  //  latts.push_back(std::vector<int> ({96,48,48,48}) );
  latts.push_back({});
  latts[0] = GridDefaultLatt();
#endif
  ...
  std::cout<<GridLogMessage << "=========================================================================="<<std::endl;
  std::cout<<GridLogMessage << "= Benchmarking Stag"<<std::endl;
  std::cout<<GridLogMessage << "=========================================================================="<<std::endl;
  std::cout<<GridLogMessage << "Volume \t\t\tProcs \t SchurStaggered "<<std::endl;
  std::cout<<GridLogMessage << "=========================================================================="<<std::endl;

  for (int l=0;l<latts.size();l++){
    Coordinate latt4 = latts[l];
...
void benchSt(Coordinate & latt4, int Ls)
{
  /////////////////////////////////////////////////////////////////////////////////////
  // for Nc=3
  /////////////////////////////////////////////////////////////////////////////////////
  ...
  ImprovedStaggeredFermionF Ds(Umu,Umu,*UGrid,*UrbGrid,mass,c1,c2,u0,params);
  
  //LatticeFermionF src_o (UrbGrid); src_o=1.0;
  //LatticeFermionF r_o   (UrbGrid); r_o=Zero();
  LatticeColourVectorF src_o (UrbGrid); src_o=1.0;
  LatticeColourVectorF r_o   (UrbGrid); r_o=Zero();

  int order =401;
  SchurStaggeredOperator<ImprovedStaggeredFermionF,LatticeColourVectorF>  Mpc(Ds); //LatticeFermionF>  Mpc(Ds);
  Chebyshev<LatticeColourVectorF>      Cheby(0.0002,24.0,order);//LatticeFermionF>      Cheby(0.0,60.0,order);
